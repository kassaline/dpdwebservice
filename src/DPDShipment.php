...
class DPDShipment
{
    ...
    public function addParcel(array $array)
    {
        ...
        $parcel['weight'] = (int)ceil($array['weight'] / 10);

        for ($i = 1; $i <= 4; $i++) {
            if (isset($array['referenz' . $i])) {
                $parcel['customerReferenceNumber' . $i] = $array['referenz' . $i];
            }
        }

        $this->storeOrderMessage['order']['parcels'][] = $parcel;
    }
